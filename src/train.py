...
Trains THREE classifiers on the Pima diabetes data, evaluates each with five
metrics, logs everything to MLflow, prints a comparison table, then:
...
Run:  py -3.13 src/train.py       (or: python src/train.py inside the venv)
"""
import os
import sys
import json
...
from utils import FEATURES, TARGET, load_data, zero_to_nan, default_data_path  # noqa: E402
...
def build_pipeline(classifier, scale: bool) -> Pipeline:
    ...
    values straight in -- the pipeline cleans/imputes/scales automatically, the
    exact same way it did during training.

    Steps:
      1. zeros -> NaN   (impossible zeros become missing)
      2. impute median  (fill the missing values)
      3. scale          (only for Logistic Regression, which is sensitive to
                         feature scale; tree models don't need it)
    """
    steps = [
        ("zeros", FunctionTransformer(zero_to_nan)),
...
def evaluate(model, X_test, y_test) -> dict:
    ...
    proba = model.predict_proba(X_test)[:, 1]  # P(diabetic) for ROC-AUC
    return {
...
def main():
    ...
    for name, cfg in model_configs().items():
        with mlflow.start_run(run_name=name):
            ...
            # Log the fitted pipeline. cloudpickle (not the newer skops default)
            # is needed because our pipeline contains a custom
            # FunctionTransformer (zero_to_nan). The returned info gives a stable
            # models:/ URI we use for registration and reloading.
