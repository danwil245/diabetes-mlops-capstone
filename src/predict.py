...
def predict(features: dict) -> dict:
    """Predict diabetes from a dict of the 8 feature values.

    Returns the class (0/1), a human label, and P(diabetic). The pipeline does
    all cleaning/imputation/scaling internally, so raw values go straight in.
    """
    model = load_model()
